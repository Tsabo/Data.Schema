...
public sealed class PostgresSchemaValidator : ISchemaValidator
{
    private static readonly HashSet<string> _validTypes = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        "BYTEA", "CHARACTER", "CHAR", "VARCHAR", "TEXT",
        "CIDR", "INET", "MACADDR", "MACADDR8",
        "DATE", "FLOAT4", "FLOAT8", "DOUBLE PRECISION", "REAL",
        "INTEGER", "INT", "INT4", "INTERVAL",
        "JSON", "JSONB",
        "MONEY", "NUMERIC", "DECIMAL",
        "OID", "PATH", "POINT", "POLYGON",
        "SERIAL", "SERIAL4", "SMALLINT", "INT2", "SMALLSERIAL", "SERIAL2",
        ...
        foreach (var table in schema.Tables)
        {
            ...
            foreach (var item in table.Columns)
            {
                ...
                if (!_validTypes.Contains(baseType))
                    result.Errors.Add($"Unsupported PostgreSQL type '{item.Type}' for column '{table.Name}.{item.Name}'.");
            }
